...
class ProductionOllamaHandlingFlow(FlowSpec):
    ...
    def start(self):
        ...
        for request_id in range(self.max_requests):
            ...
            if total_requests >= 10:
                ...
                if success_rate < 0.3:
                    print(f"[@prod] Early exit: Success rate too low ({success_rate:.1%})")
                    break
        ...
        print(f"[@prod] Processing complete: {successful_requests} successful, {failed_requests} failed")
        self.next(self.end)
        ...
        for attempt in range(max_retries):
            try:
                ...
                if "Circuit Breaker" in error_msg:
                    ...
                    print(f"[@prod] Request {request_id}, attempt {attempt + 1}: Circuit breaker open")
                    ...
                    print(f"[@prod] Request {request_id}: Non-recoverable runtime error: {error_msg}")
                    return False
                    
            except Exception as e:
                # Unexpected error - log and retry with exponential backoff
                print(f"[@prod] Request {request_id}, attempt {attempt + 1}: {type(e).__name__}: {e}")
    ...
    def end(self):
        ...
        print("[@prod]   5. Track circuit breaker hits for monitoring (Automatic in ollama_status card.)")
        print("[@prod] ")
        ...
        if success_rate > 0.95:
            ...
            print("[@prod] 🚨 Low success rate - investigate system issues before production.")
