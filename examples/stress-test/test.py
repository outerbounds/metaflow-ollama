...
class DeadlockRootCauseAnalysis(FlowSpec):
    ...
    def start(self):
        ...
        print("[@debug] Target: Trigger deadlock around 650 requests and capture diagnostics")
        ...
        while request_count < max_requests:
            try:
                ...
                with timeout(30):  # 30 second timeout per request
                    response = chat(
                        model='qwen3:0.6b',
                        messages=[{"role": "user", "content": f"Request {request_count}: Count from 1 to 5."}],
                        stream=False,
                ...
                if request_count % 50 == 0:
                    ...
                    print(f"[@debug] Progress: {request_count} requests in {elapsed_minutes:.1f} minutes")
                    self._capture_system_snapshot(request_count, f"progress_{request_count}")
                ...
                print(f"[@debug] Deadlock occurred at request {request_count} (expected ~650)")
        ...
        self.results = {
            ...
            "deadlock_at_request": request_count if request_count < max_requests else None,
        ...
        if self.results['deadlock_occurred']:
            print(f"[@debug] Deadlock at request: {self.results['deadlock_at_request']}")
    ...
    def _capture_system_snapshot(self, request_count, snapshot_type):
        ...
        try:
            ...
            if snapshot_type in ["initial", "during_deadlock"] or request_count % 100 == 0:
                print(f"[@debug] Snapshot {snapshot_type}: RAM {snapshot['memory']['percent']:.1f}%, "
    ...
    def _capture_detailed_snapshot(self, request_count, inference_time):
        ...
        try:
            ...
            print(f"[@debug] Detailed snapshot {request_count}: "
    ...
    def _get_gpu_info(self):
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                return {"gpus": gpus, "utilization": gpus[0]["utilization"] if gpus else 0}
            else:
    ...
    def _get_process_info(self):
        ...
        return {
            ...
            "high_memory_processes": [p for p in all_processes if p['memory_info'] and p['memory_info'].rss > 1024*1024*100]  # >100MB
        }
    ...
    def _get_ollama_process_details(self):
        ...
        ollama_details = []
        for proc in psutil.process_iter(['pid', 'name', 'memory_info', 'cpu_percent', 'num_threads', 'open_files']):
            try:
    ...
    def _get_full_process_tree(self):
        ...
        try:
            processes = []
            for proc in psutil.process_iter(['pid', 'ppid', 'name', 'memory_info', 'num_threads']):
                try:
    ...
    def _capture_ollama_logs(self):
        ...
        try:
            ...
            result = subprocess.run(['journalctl', '-u', 'ollama', '--since', '5 minutes ago'], 
    ...
    def _analyze_resource_trends(self):
        ...
        print(f"[@debug] Memory usage: {memory_usage[0]:.1f}% → {memory_usage[-1]:.1f}% (Δ{memory_growth:+.1f}%)")
        
        # Process count trend
        process_counts = [len(s['processes']['ollama']) for s in self.resource_snapshots]
        if process_counts:
            print(f"[@debug] Ollama processes: {process_counts[0]} → {process_counts[-1]} (Δ{process_counts[-1] - process_counts[0]:+d})")
        ...
        if self.diagnostics:
            ...
            if len(detailed_snapshots) > 1:
                ...
                ram_growth = last['memory_detail']['percent'] - first['memory_detail']['percent']
                ...
                if fd_growth > 100:
                    print(f"[@debug] ⚠️  FILE DESCRIPTOR LEAK DETECTED: {fd_growth} growth")
