...
class OllamaStressTestFlow(FlowSpec):
    ...
    def concurrent_stress_test(self):
        ...
        print(
            f"[@test] Starting concurrent stress test with {self.concurrent_requests} requests"
        )
        ...
        with ThreadPoolExecutor(max_workers=self.concurrent_requests) as executor:
            ...
            for future in as_completed(futures):
                ...
                print(
                    f"[@test] Request {result['request_id']} ({'✓' if result['success'] else '✗'}): "
        ...
        print(
            f"[@test] Concurrent test complete: {len(successful_requests)}/{len(concurrent_results)} successful"
        )
        ...
        self.concurrent_results = {
            ...
            "average_duration": (
                sum(r["duration"] for r in successful_requests)
    ...
    def long_running_test(self):
        ...
        while time.time() < end_time:
            try:
                ...
                response = chat(
                    model="llama3.2:3b", messages=[{"role": "user", "content": content}]
                )
    ...
    def deadlock_simulation_test(self):
            ...
            deadlock_test_results.append(
                {
                    "test": "rapid_requests",
                    "success": True,
                    "message": f"Completed {len(results)}/20 requests",
                }
            )
        except Exception as e:
            deadlock_test_results.append(
                {"test": "rapid_requests", "success": False, "error": str(e)}
            )
        ...
        try:
            ...
            def background_requests():
                for i in range(10):
                    try:
                        chat(
                            model="llama3.2:1b",
                            messages=[{"role": "user", "content": f"Background {i}"}],
                        )
                    except:
                        pass
                    time.sleep(2)
            ...
            long_response = chat(
                ...
                messages=[
                    {
                        "role": "user",
                        "content": "Write a detailed explanation of machine learning in 500 words.",
                    }
                ],
            )
            ...
            deadlock_test_results.append(
                {"test": "long_request_with_load", "success": False, "error": str(e)}
            )
        ...
        try:
            ...
            for i in range(5):
                ...
                chat(
                    model="llama3.2:1b",
                    messages=[{"role": "user", "content": f"Health test {i}"}],
                )
                time.sleep(1)

            deadlock_test_results.append(
                {
                    "test": "health_during_load",
                    "success": True,
                    "message": "Server remained healthy",
                }
            )
        except Exception as e:
            deadlock_test_results.append(
                {"test": "health_during_load", "success": False, "error": str(e)}
            )
    ...
    def end(self):
        ...
        if hasattr(self, "basic_test_passed"):
            print(
                f"✓ Basic functionality: {'PASSED' if self.basic_test_passed else 'FAILED'}"
            )
        ...
        if hasattr(self, "big_model_result"):
            ...
            print(
                f"{status} Big model test (deepseek-r1:7b): {result['duration']:.2f}s"
            )
        ...
        if hasattr(self, "concurrent_results"):
            ...
            print(
                f"✓ Concurrent requests: {success_rate:.1f}% success rate "
        ...
        if hasattr(self, "long_running_results"):
            ...
            print(
                f"  Requests: {lr['total_requests']} ({lr['success_rate']*100:.1f}% success)"
            )
        ...
        if hasattr(self, "deadlock_test_results"):
            ...
            print(
                f"✓ Deadlock simulation: {len(successful_tests)}/{len(dt['results'])} tests passed"
            )
